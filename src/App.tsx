...
function App() {
    ...
    const firstCall = async () => {
        let response = await fetch(`${urlPrefix}/random?size=${1}`, { // https://api.openbrewerydb.org/breweries/random?size=3
            mode: "cors",
        });
    ...
    console.log("HERE !")
      ...
      let response = await fetch(`${urlPrefix}${urlSuffix}`, { mode: "cors" });
      let refinedResponse = await response.json();
      setMemory([...refinedResponse])
  };
    ...
    return (
        <div className="App">
            ...
            <button onClick={()=>{console.log(memory)}} >Console data</button>
